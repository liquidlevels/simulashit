...
class App(tk.Tk):
    def __init__(self):
        ...
        self.parametro_media = tk.DoubleVar() # Valor de la media
    ...
    def crear_widgets(self):
        ...
        #
        ttk.Label(datos, text="Indique el parámetro de escala (media)", justify=LEFT,
        background="#C1E1C1").pack(anchor=tk.W, padx=10, pady=5, fill=tk.X)

        media = Entry(datos, textvariable=self.parametro_media)
        ...
        justify=LEFT, background="#C1E1C1").pack(anchor=tk.W, padx=10, pady=5, fill=tk.X)
    ...
    def simular(self):
        valores = np.random.exponential(
            self.parametro_media.get(),
            [10, 5, 365]
        )
        ...
        suma = 0
        if tipo_calculo == 1:
            for j in valores:suma = suma + 1 if j < self.valor_minimo.get() else suma + 0
        elif tipo_calculo == 2:
            for j in valores:
    ...
    def calcular(self):
        bandera = 0
        if not self.parametro_media.get():
            messagebox.showerror("Error de ingreso", "Se debe declarar a la media")
        else:
            if self.parametro_media.get() <= 0: 
                messagebox.showerror("Error de desviación", "El promedio no puede ser negativo")
            else:
                bandera += 1
        if not self.simulacion.get():
            messagebox.showerror("Selección de probabilidad", "Debe indicar el tipo de cálculo a realizar")
        else:
            bandera += 1
            tipo_calculo = self.lectura(self.simulacion.get())
            if tipo_calculo == 5:
                if not self.valor_maximo.get():
                    messagebox.showerror("Declaración de intervalo","Debe indicar el valor final del intervalo")
                else:
                    if self.valor_maximo.get() <= self.valor_minimo.get(): 
                        messagebox.showerror("Declaración de intervalo", "El valor final no puede ser menor al valor inicial")
                    else:
                        bandera += 2
            else:
                if not self.valor_minimo.get():
                    messagebox.showerror("Cálculo de probabilidad", "Requiere indicar el valor por calcular")
                else:
                    bandera += 1
